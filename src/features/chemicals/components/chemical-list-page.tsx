// src/features/chemicals/components/chemical-list-page.tsx
...
function ChemicalHistoryModal({
  ...
  // Assumes backend sorts newest first, so the latest version is the first item.
  const latestVersion = chemical.versionHistory[0];
  ...
  return (
    <Dialog open={!!chemical} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-4xl lg:max-w-6xl xl:max-w-7xl w-[90vw]">
        <DialogHeader>
          <DialogTitle>Version History for: {latestVersion.name}</DialogTitle>
          <DialogDescription>
            Showing all changes for this chemical inventory item. Newest changes are at the top.
          </DialogDescription>
        </DialogHeader>
              ...
              <TableBody>
                {chemical.versionHistory.map((history: VersionHistory) => (
                  <TableRow key={history.id}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">{history.status.replace(/_/g, ' ')}</Badge>
                    </TableCell>
                    <TableCell>{history.location}</TableCell>
                    {/* NEW/UPDATED Cells */}
                    <TableCell>{`${history.volumeOrMass} ${history.unit}`}</TableCell>
                    <TableCell className="font-mono">
                      H:{history.health} F:{history.flammability} I:{history.instability}
                    </TableCell>
...
// =======================================================================
// === ChemicalListPage Main Component (largely unchanged) ===
// =======================================================================
export default function ChemicalListPage() {
  ...
  const { data, loading, error } = useQuery<GetAllChemicalsData, GetAllChemicalsVars>(
    GET_ALL_CHEMICALS,
